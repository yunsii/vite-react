...
>(
  ...
  >
  return useConcent<Record<string, unknown>, Ctx>(
    registerOptions,
    concentClassKey,
  )
}
...
>(
  setup: T,
  options?: OptionsBase<Props, ComputedDesc, Extra, StaticExtra, MapProps>,
) {
  ...
  type Ctx = ModuleContext<
    ...
    [Extra, StaticExtra, ReturnType<MapProps>]
  >
  const { settings } = useConcent<Record<string, unknown>, Ctx>(
    registerOptions,
    concentClassKey,
  )
  return settings
 ...
 * @param args
 */
export function concentEmit<E extends EventKeys, T extends EventMap[E]>(
  eventName: E,
  ...args: T
) {
  emit(eventName, ...args)
